...
public class CartDAOImpl implements CartDAO, Serializable{
	...
	@Transactional
	public Long getTotalAmount(String user_email) {
		String hql = "select sum(price) from Cart where user_email= " + "'" + user_email + "' and ordered = 0";
		Query query = sessionFactory.getCurrentSession().createQuery(hql);
	...
	@Transactional
	public List<Cart> getActiveByUser(String user_email) {
		String hql = "from Cart where user_email= " + "'" + user_email + "' and ordered = 0";
		Query query = sessionFactory.getCurrentSession().createQuery(hql);
		return query.list();
	}
